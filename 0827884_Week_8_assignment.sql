...
ORDER BY daysSelect month([start_date]) as mon, DAY([start_date]) as dom,	sum(Case when year([start_date]) = 2004 then 1 else 0 end) as n2004,	sum(Case when year([start_date]) = 2005 then 1 else 0 end) as n2005,	sum(Case when year([start_date]) = 2006 then 1 else 0 end) as n2006from Subscribers swhere year([start_date]) in (2004, 2005, 2006)group by month([start_date]), DAY([start_date])order by mon, domSelect month([start_date]) as month,	sum(Case when year([start_date]) = 2004 then 1 else 0 end) as n2004,	sum(Case when year([start_date]) = 2005 then 1 else 0 end) as n2005,	sum(Case when year([start_date]) = 2006 then 1 else 0 end) as n2006from Subscribers swhere year([start_date]) in (2004, 2005, 2006)group by month([start_date])order by month